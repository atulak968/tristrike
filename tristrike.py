...
def run_pipeline(target: str, scope: list = None, output_dir: str = "reports_output"):
    ...
    # 1. Reconnaissance Agent (HexStrike style)
    ...
    # 2. Crawler & Discovery Agent (Agentic Bug Hunter style)
    print("\n[+] [2/4] Running Discovery & Crawler Agent (Routes, Parameters, Hidden Paths)...")
    crawler = CrawlerEngine(scope_mgr)
    crawl_data = crawler.crawl(target)
    print(f"    -> Crawled Endpoints: {len(crawl_data['all_endpoints'])}")
    print(f"    -> Endpoints with Parameters: {len(crawl_data['param_endpoints'])}")
    print(f"    -> Sensitive/Hidden Paths: {len(crawl_data['hidden_paths'])}")

    # 3. Active Vulnerability Agent (SQLi, XSS, Heuristics)
    ...
    if not test_targets and crawl_data['all_endpoints']:
        # Heuristic parameter testing if no explicit queries found
        test_targets = [f"{crawl_data['all_endpoints'][0]}?q=test", f"{crawl_data['all_endpoints'][0]}?id=1"]

    for ep in test_targets[:10]:
        print(f"    Testing target: {ep[:70]}...")
        raw_findings.extend(vuln_engine.test_sqli(ep))
        raw_findings.extend(vuln_engine.test_xss(ep))

    print(f"    -> Raw Candidates Discovered: {len(raw_findings)}")

    # 4. Skeptical Reviewer & Reality Gate (BugTraceAI style)
    ...
    for f in raw_findings:
        ...
        if is_valid:
            ...
            print(f"    [VERIFIED TRUE POSITIVE] {f['title']} (Confidence: {meta['confidence_score']}%)")
        else:
